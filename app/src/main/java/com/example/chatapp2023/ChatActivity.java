...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new MyAdapter(chatArrayList, stEmail, user);
        ...
        myRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                Log.d("Firebase", "##############################실행완료");
                // 데이터를 한 번만 읽어옴
                // dataSnapshot에서 원하는 데이터를 가져와서 처리
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    Log.d("Firebase", "##############################실행완료2");

                    user = snapshot.getValue(User.class);
                    if(stEmail.equals(user.getUserEmail())) {
                        String uuser = user.getUserEmail();
                        String pwd = user.getSecondpwd();

                        Log.d("Firebase", "Value is: " + uuser);
                        Log.d("Firebase", "Value is: " + pwd);
                        break;
                    }else{
                        Log.d("Firebase", "##############################실행완료4");
                        user.userEmail = "";
                        user.secondpwd = "";
                    }
                }
        ...
        ChildEventListener childEventListener = new ChildEventListener() {
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String previousChildName) {
                ...
                chat.setChat_id(dataSnapshot.getKey());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 2)   {
            if (resultCode == RESULT_OK) {
                ...
                if (currentTime - lastClickTime < 1000) { // 1초 동안 중복 클릭 무시
                    ...
                    int options = data.getIntExtra("option",-1);
                    int chat_position = data.getIntExtra("position", -1);

                    if(options == 1) { //첫번째 옵션 선택 즉 메시지 확인 선택 시
                        Intent intent = new Intent(ChatActivity.this, PasswdActivity.class);

                        intent.putExtra("position", chat_position); //받아온 결과  Passwd엑티비티에 전송
                        startActivityForResult(intent, 3);
                    }

                    else if (options == 2) {//두번째 옵션 선택 즉 메시지 상태 변경 중 잠금 선택 시
                        Intent intent = new Intent(ChatActivity.this, PasswdActivity.class);
                        intent.putExtra("position", chat_position); //받아온 결과  Passwd엑티비티에 전송
                        startActivityForResult(intent, 4);
                    }

                    else if (options == 3) {//세번째 옵션 선택 즉 메시지 상태 변경 중 잠금 해제 선택 시
                        Intent intent = new Intent(ChatActivity.this, PasswdActivity.class);
                        intent.putExtra("position", chat_position); //받아온 결과  Passwd엑티비티에 전송
        ...
        if (requestCode == 4){ // 메시지 잠금 해제
            if (resultCode == RESULT_OK) {
                // PasswdActivity에서 성공적으로 작업을 완료하고 돌아온 경우
                ...
                if (chat_position != -1){
                    ...
                    DatabaseReference ref = database.getReference("message");
                    DatabaseReference ChatRef = ref.child(chat.chat_id);

                    ChatRef.child("check_hide").setValue("no");
                    chat.setCheck_hide("no");
                    mAdapter.notifyDataSetChanged();
        ...
        if (requestCode == 5){ // 메시지 잠금
            if (resultCode == RESULT_OK) {
                ...
                if (chat_position != -1){
                    ...
                    ChatRef.child("check_hide").setValue("yes");
                    chat.setCheck_hide("yes");
                    mAdapter.notifyDataSetChanged();

                    Toast.makeText(ChatActivity.this, "잠금에 성공했습니다.", Toast.LENGTH_LONG).show();
                }else{
                    Toast.makeText(ChatActivity.this, "잠금에 실패했습니다.", Toast.LENGTH_LONG).show();
                }
            }
        }
    }
